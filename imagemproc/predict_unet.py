...
def main(file_bytes):
    ...
    for enumeration_id, enumeration_bbox in enumeration_prediction_bbox_dict.items():
        image = cv2.rectangle(image, (enumeration_bbox[0], enumeration_bbox[1]), (enumeration_bbox[0]+enumeration_bbox[2],
                              enumeration_bbox[1]+enumeration_bbox[3]), tooth_colormap[enumeration_id-1]["color"], thickness)
        image = cv2.putText(image, str(tooth_colormap[enumeration_id-1]["name"]), (enumeration_bbox[0], enumeration_bbox[1]-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, tooth_colormap[enumeration_id-1]["color"], thickness, cv2.LINE_AA)
    return image
